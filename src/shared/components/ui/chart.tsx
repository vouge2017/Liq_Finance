...
// Charts are only used in lazy-loaded pages (BudgetPage), so this is safe
...
const ChartTooltip = React.forwardRef<any, any>((props, ref) => {
  ...
  React.useEffect(() => {
    loadRecharts().then((recharts) => {
      setRechartsTooltip(() => recharts.Tooltip)
      setLoading(false)
    })
  }, [])

  if (loading || !RechartsTooltip) {
    return null // Return null during loading to avoid layout shift
  }

  return <RechartsTooltip {...props} ref={ref} />
})
...
const ChartLegend = React.forwardRef<any, any>((props, ref) => {
  ...
  React.useEffect(() => {
    loadRecharts().then((recharts) => {
      setRechartsLegend(() => recharts.Legend)
      setLoading(false)
    })
  }, [])

  if (loading || !RechartsLegend) {
    return null // Return null during loading
  }

  return <RechartsLegend {...props} ref={ref} />
})
