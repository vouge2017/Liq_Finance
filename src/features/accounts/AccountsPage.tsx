    ...
    <div className="pb-28 animate-push relative">
      ...
      <div className="mb-6 flex flex-col gap-2">
         ...
         <div className="w-full h-3 bg-theme-card rounded-full flex overflow-hidden">
            <div className="h-full bg-purple-500" style={{ width: totalAssets > 0 ? `${(bankAssets/totalAssets)*100}%` : '0%' }}></div>
            <div className="h-full bg-cyan-500" style={{ width: totalAssets > 0 ? `${(mobileAssets/totalAssets)*100}%` : '0%' }}></div>
            ...
            {modalStep === 'form' ? (
                ...
                {(!editingAccount && !selectedInstitution) && (
                    <div className="grid grid-cols-3 gap-3">
                        {(['Bank', 'Mobile Money', 'Cash'] as const).map(t => (
                            <button 
                            key={t}
                            onClick={() => setNewAccType(t)}
                            className={`flex flex-col items-center justify-center p-4 rounded-2xl border transition-all duration-200 ${newAccType === t ? 'bg-cyan-500/10 border-cyan-500 text-cyan-400 scale-105 shadow-lg shadow-cyan-500/10' : 'bg-theme-main border-theme text-theme-secondary hover:bg-theme-main/80'}`}
                            >
                                <div className={`mb-2 ${newAccType === t ? 'text-cyan-400' : 'text-theme-secondary'}`}>
                ...
                {(!selectedInstitution) && (
                    <div className={editingAccount ? 'opacity-50 pointer-events-none' : ''}>
                        <label className="text-xs text-theme-secondary block mb-2 font-medium">Institution Name</label>
                        <input 
                        type="text" 
                        ...
                        placeholder={newAccType === 'Bank' ? "e.g. CBE, Dashen, Awash" : "e.g. Telebirr, MPesa"}
                        />
                    </div>
                )}
