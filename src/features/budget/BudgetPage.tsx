  ...
  return (
    <div className="pb-28 animate-fade-in space-y-6">
        ...
        {calculatedBudget.isDeficit ? (
             // --- DEFICIT STATE (RED) ---
             <div className="bg-gradient-to-r from-rose-900 to-red-900 rounded-3xl p-6 border border-rose-800/50 shadow-lg relative overflow-hidden animate-pulse-subtle">
                ...
                <div className="relative z-10">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-2 mb-1">
                            <Icons.Alert className="text-rose-300" size={16} />
                            <span className="text-xs font-bold text-rose-100/90 uppercase tracking-wider">Budget Deficit</span>
                        </div>
                        
                        {/* AI Action */}
                        <button 
                            onClick={() => setActiveTab('ai')}
                            className="flex items-center gap-1 bg-rose-800/50 hover:bg-rose-800 px-3 py-1.5 rounded-full border border-rose-700/50 transition-colors"
                        >
                            <Icons.AI size={12} className="text-rose-200" />
                            <span className="text-[10px] font-bold text-rose-100">Get Help</span>
                        </button>
                    </div>

                    <div className="flex items-baseline gap-2 mb-2">
                        <span className="text-lg text-rose-200">Over by</span>
                        <span className="text-4xl font-bold text-white tracking-tight">
                            {isPrivacyMode ? '••••' : Math.abs(calculatedBudget.remainingVar).toLocaleString()}
                        </span>
                        <span className="text-sm font-medium text-rose-200/80">ETB</span>
                    </div>
                    
                    <p className="text-xs text-rose-200/70 mb-4">
                        Recovery Target: Save <span className="font-bold">{Math.round(Math.abs(calculatedBudget.safeDaily)).toLocaleString()} ETB/day</span> to break even.
                    </p>

                    <button 
                        onClick={() => setShowFixModal(true)}
                        className="w-full py-3 bg-white text-rose-900 font-bold rounded-xl shadow-lg hover:bg-rose-50 transition-colors flex items-center justify-center gap-2"
                    >
                        <Icons.Repeat size={16} /> Fix My Budget
                    </button>
                </div>
            </div>
        ) : (
            // --- HEALTHY STATE (GREEN) ---
            <div className="bg-gradient-to-r from-teal-900 to-emerald-900 rounded-3xl p-6 border border-teal-800/50 shadow-lg relative overflow-hidden">
                <div className="absolute top-0 right-0 w-40 h-40 bg-cyan-400/10 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none"></div>
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs font-bold text-teal-100/70 uppercase tracking-wider">Daily Safe to Spend</span>
                            </div>
                            <p className="text-xs text-teal-200/60">{calculatedBudget.daysRemaining} days left</p>
                        </div>
                        
                        {/* AI Action */}
                        <button 
                            onClick={() => setActiveTab('ai')}
                            className="flex items-center gap-1 bg-teal-800/50 hover:bg-teal-800 px-3 py-1.5 rounded-full border border-teal-700/50 transition-colors"
                        >
                            <Icons.AI size={12} className="text-teal-200" />
                            <span className="text-[10px] font-bold text-teal-100">Analyze Spending</span>
                        </button>
                    </div>

                    <div className="flex items-baseline gap-2 mb-4">
                        <span className="text-5xl font-bold text-white tracking-tight">
                            {isPrivacyMode ? '••••' : Math.round(calculatedBudget.safeDaily).toLocaleString()}
                        </span>
                        <span className="text-lg font-medium text-teal-200/80">ETB</span>
                    </div>

                    <div className="w-full bg-black/20 rounded-full h-1.5 overflow-hidden flex">
                        <div 
                            className="h-full bg-teal-400" 
                            style={{ width: `${Math.min(100, (calculatedBudget.remainingVar / (calculatedBudget.remainingVar + calculatedBudget.variable.reduce((s,c)=>s+c.spent,0) || 1))*100)}%` }}
                        ></div>
                    </div>
                </div>
            </div>
                    ...
                    {calculatedBudget.variable.map(cat => {
                        ...
                        return (
                            <div 
                                ...
                                className="bg-theme-card p-4 rounded-2xl border border-theme relative overflow-hidden cursor-pointer active:scale-[0.99] transition-transform"
                            >
                                <div className="flex justify-between items-center mb-3 relative z-10">
                                    {/* Left Side: Icon & Text */}
                                    <div className="flex items-center gap-3 flex-1 min-w-0">
                                        <div className={`w-12 h-12 rounded-2xl flex-shrink-0 flex items-center justify-center bg-gradient-to-br ${cat.color} text-white shadow-md shadow-${cat.color.replace('bg-','')} `}>
                                            <Icon size={20} className="drop-shadow-sm" />
                                        </div>
                                        <div className="min-w-0">
                                            <h4 className="font-bold text-theme-primary truncate">{cat.name}</h4>
                                            <p className="text-[10px] text-theme-secondary">
                                                {isPrivacyMode ? '••••' : cat.spent.toLocaleString()} {isUncategorized ? '' : `/ ${isPrivacyMode ? '••••' : cat.allocated.toLocaleString()}`}
                                            </p>
                                        </div>
                                    </div>
                                    
                                    {/* Right Side: Stats + Quick Add - SPACED & SEPARATED */}
                                    <div className="flex items-center gap-4 shrink-0 ml-2">
                                        <div className="flex flex-col items-end">
                                            {isUncategorized ? (
                                                <span className="text-xs font-bold text-theme-secondary">--</span>
                                            ) : (
                                                <span className={`text-xs font-bold ${isOver ? 'text-rose-500' : 'text-theme-primary'}`}>
                                                    {Math.round(percent)}%
                                                </span>
                                            )}
                                            {cat.rolloverEnabled && <Icons.Repeat size={12} className="text-cyan-400 mt-1" />}
                                        </div>
                                        {!isUncategorized && (
                                            <button 
                                                onClick={(e) => openQuickAdd(e, cat.name, 'expense')}
                                                className="w-10 h-10 flex items-center justify-center rounded-full bg-theme-main border border-theme text-theme-secondary hover:text-cyan-400 hover:border-cyan-500 transition-colors shadow-sm z-20"
                                ...
                                {/* Progress Bar */}
                                <div className="w-full h-2 bg-theme-main rounded-full overflow-hidden relative z-10">
                                    <div 
                    ...
                    {calculatedBudget.fixed.map(cat => {
                        ...
                        return (
                            <div 
                                key={cat.id} 
                                onClick={() => setDetailCatId(cat.id)}
                                className="bg-theme-card p-5 rounded-3xl border border-theme cursor-pointer relative overflow-hidden group hover:border-cyan-500/30 transition-all shadow-sm active:scale-[0.98] flex flex-col justify-between min-h-[170px]"
                            >
                                ...
                                <div className="flex justify-between items-start relative z-10">
                                    ...
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center bg-gradient-to-br ${cat.color} text-white shadow-lg ring-1 ring-white/10 backdrop-blur-sm group-hover:scale-110 transition-transform duration-300`}>
                                    ...
                                    <button 
                                        onClick={(e) => openQuickAdd(e, cat.name, 'expense')}
                                        className="w-10 h-10 flex items-center justify-center rounded-full bg-theme-main/80 backdrop-blur-sm border border-theme text-theme-secondary hover:text-cyan-400 hover:border-cyan-500 transition-colors shadow-sm"
                                        title="Quick Add Expense"
                                    >
                                        <Icons.Plus size={18} />
                                    </button>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4 relative z-10">
                                    {/* Realistic Icon */}
                                    <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${iconObj.color} flex items-center justify-center text-white shadow-lg ${iconObj.shadow || ''} ring-1 ring-white/10`}>
                                        <iconObj.icon size={20} className="drop-shadow-sm" />
                                    </div>
                                    <div>
